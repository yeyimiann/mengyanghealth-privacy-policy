...
**1.5** **索取权限的规则说明**：
蓝牙权限和设备位置信息：
支持设备与应用的连接管理，查看设备测量数据与设备电量，您可以通过App设置设备的参数，例如血氧饱和度提醒阈值、震动提醒等。
相机相册权限：
用于扫描设备分享二维码绑定分享设备。
...
**5.2** **数据删除**：用户可以随时联系客服请求删除其账户和数据，数据将在删除请求处理后从服务器中移除（除非法律要求保留）。您也可直接通过App内【我的】→【账户与安全】→【注销账号】路径直接注销账号与其相关数据。
